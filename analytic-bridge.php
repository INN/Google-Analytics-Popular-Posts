...
/*
 * Define table name constants used throughout
 */
...
function largo_anaylticbridge_cron($verbose=false) {
	...
	if ( $verbose ) {
		echo("Analytic Bridge cron executed successfully\n");
		echo("\nEnd analyticbridge_cron\n");
	}
...
function query_and_save_analytics($analytics, $startdate, $verbose=false) {
	...
	try {
		...
		foreach ( $iter as $r ) {
			 ...
			 */
			$pagesql .= $wpdb->prepare( "\t(%s, %s)", $gapath, $postid );
